...
public class GetSettingsFunction : WebFunction<GetSettingsFunctionResponse>
{
    protected override async Task<GetSettingsFunctionResponse> Handle(IServiceProvider provider)
    {
        ...
        var shouldHighlightMessages = await config.Get(EConfigOptions.UserHighlight, "False") == "True";
        var language = await config.Get(EConfigOptions.Language, "en");
        var alertOnMention = await config.Get(EConfigOptions.AlertOnMention, "True") == "True";
        var alertOnKeyword = await config.Get(EConfigOptions.AlertOnKeyword, "False") == "True";
        var highlightOnKeyword = await config.Get(EConfigOptions.HighlightOnKeyword, "False") == "True";
        var keywordList = await config.Get(EConfigOptions.KeywordList) ?? "";
        var showBeatmapBanner = await config.Get(EConfigOptions.ShowBeatmapBanner, "True") == "True";
